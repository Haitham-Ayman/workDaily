...
const db = new sqlite3.Database(path.join(__dirname, 'tasks.db'), (err) => {
    if (err) {
        ...
        console.log('Connected to SQLite database');
        // Create completed_tasks table with AUTOINCREMENT
        db.run(`CREATE TABLE IF NOT EXISTS completed_tasks (
...
// Function to save completed tasks (without manually providing id)
function saveCompletedTasks(tasks) {
    return new Promise((resolve, reject) => {
        const stmt = db.prepare(`INSERT INTO completed_tasks (text, notes, completed_at, created_at) 
                                 VALUES (?, ?, ?, ?)`);
